...
for(yr in 1995:2024){
  ...
  fav <- fav[,-which(colnames(fav) %in% c("pLev", "land", "west", "east", "central", "col_num"))] #remove unneeded columns
  ...
  favL$seas <- ifelse(as.character(favL$day) < "X0701", "spr", "fal")
  ...
  for(i in allID) {
    #i = allID[1]
    if(i %% 500 == 0) {print(paste0(i, " of ", max(allID)))}
...
png(file = "Favorable_wind_intervals.png", width = 3, height = 6, units = "in", res = 300)
grid.arrange(uE, uC, uW, nrow = 3)
dev.off()
